...
function incrementarPorUno(array) {
  ...
  // Tu código:
  let newArray = [];
  for(let i = 0; i < array.length; i++) newArray[i] = array[i] + 1; 
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  // let producto = 1;
  // for(let i = 0; i < arguments.length; i++) producto *= arguments[i];
  if(arguments.length === 0) return 0;
  else return [...arguments].reduce((prev, curr) => prev * curr);
}
...
function todosIguales(arreglo) {
  ...
  for(let i = 1; i < arreglo.length; i++){
    if(element !== arreglo[i]) return false;
  }
...
function mesesDelAño(array) {
  ...
  if(meses.includes('Enero', 'Marzo', 'Noviembre')) return meses;
...
function mayorACien(array) {
  ...
  // Tu código:
  let mayor = array.filter(numero => numero > 100);
  return mayor;
}
